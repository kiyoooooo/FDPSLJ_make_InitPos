...
int main()
{
  ...
  const double angle_to_place_inner_lipid = M_PI / (double)(num_inner_lipid_part); //ベシクルをの内側を構成する脂質の角度間隔
  const double angle_to_place_outer_lipid = M_PI / (double)(num_outer_lipid_part); //ベシクルをの外側を構成する脂質の角度間隔

  const int num_inner_lipid_xy = (int)(M_PI / angle_to_place_inner_lipid);
  const int num_inner_lipid_xz = (int)((2.0 * M_PI) / angle_to_place_inner_lipid);

  const int num_outer_lipid_xy = (int)(M_PI / angle_to_place_outer_lipid);
  const int num_outer_lipid_xz = (int)((2.0 * M_PI) / angle_to_place_outer_lipid);
  //  const int num_inner_lipid = ((int)(M_PI / angle_to_place_inner_lipid)) * ((int)((2.0 * M_PI) / angle_to_place_inner_lipid)); //ベシクルの内側を構成する脂質の数
  //  const int num_outer_lipid = ((int)(M_PI / angle_to_place_outer_lipid)) * ((int)((2.0 * M_PI) / angle_to_place_outer_lipid)); //ベシクルの外側を構成する脂質の数

  const int num_inner_lipid = num_inner_lipid_xy * num_inner_lipid_xz; //ベシクルの内側を構成する脂質の数
  const int num_outer_lipid = num_outer_lipid_xy * num_outer_lipid_xz; //ベシクルの外側を構成する脂質の数
  ...
  const int num = num_inner_water + num_outer_water + num_lipid;                                     //すべての粒子数
  ...
  //脂質の配置を設定数配置するまで置き続けるようにした．
  int current_inner_lipid_delete_num = 0;
  int current_outer_lipid_delete_num = 0;
  double current_rand = 0;
  double current_rand0 = 0;
  double current_rand1 = 0;
  double current_rand2 = 0;
  double current_rand3 = 0;
  double current_rand4 = 0;

  //lipidの配置
  //ベシクル内側を構成するlipid
  int count_num_inner_lipid_xy = 0;
  int count_num_inner_lipid_xz = 0;
  int count_num_outer_lipid_xy = 0;
  int count_num_outer_lipid_xz = 0;
  temp_angle_xy = 0;
  temp_angle_xy = 0;

  for (count_num_inner_lipid_xy = 0; count_num_inner_lipid_xy < num_inner_lipid_xy; count_num_inner_lipid_xy++)
  {
    temp_angle_xy += angle_to_place_inner_lipid;
    for (count_num_inner_lipid_xz = 0; count_num_inner_lipid_xz < num_inner_lipid_xz; count_num_inner_lipid_xz++)
    {
...
//      current_rand = get_rand_uni_real(engine);
      ...
      if (std::pow(size_ratio * (1.0 - std::cos(2.0 * temp_angle_xz)), power_ratio) * current_rand <= lipid_delete_rate && (1.0 / 2.0) * (1.0 - std::cos(2.0 * temp_angle_xz)) != 1.0)
      { //密度の高い部分を改善するため
        //疎水基3_write_pos
      ...
      {
        current_rand0 = get_rand_uni_real(engine);
        while ((current_rand0 > 0.2 && current_rand0 < 0.3) || (current_rand0 > 0.7 && current_rand0 < 0.8))
        {
          current_rand0 = get_rand_uni_real(engine);
        }
        current_rand2 = get_rand_uni_real(engine);

        //疎水基3_write_pos
        fprintf(fpo0, "%d %d   %lf   %lf   %lf \n", particle_id, type_lipid_b,
                (vesicle_core_pos_x + vesicle_radius - lipid_to_lipid_length / 2) * std::cos(current_rand0 * 2 * M_PI) * std::cos(current_rand2 * M_PI),
                (vesicle_core_pos_y + vesicle_radius - lipid_to_lipid_length / 2) * std::sin(current_rand2 * M_PI) * std::cos(current_rand0 * 2 * M_PI),
                (vesicle_core_pos_z + vesicle_radius - lipid_to_lipid_length / 2) * std::sin(current_rand0 * 2 * M_PI));
        //疎水基3_write_vel
        fprintf(fpo1, "%d   %lf   %lf   %lf \n", particle_id, 0.0, 0.0, 0.0);

        //疎水基2_write_pos
        fprintf(fpo0, "%d %d   %lf   %lf   %lf \n", particle_id + 1, type_lipid_b,
                (vesicle_core_pos_x + vesicle_radius - lipid_to_lipid_length / 2 - bond_length) * std::cos(current_rand0 * 2 * M_PI) * std::cos(current_rand2 * M_PI),
                (vesicle_core_pos_y + vesicle_radius - lipid_to_lipid_length / 2 - bond_length) * std::sin(current_rand2 * M_PI) * std::cos(current_rand0 * 2 * M_PI),
                (vesicle_core_pos_z + vesicle_radius - lipid_to_lipid_length / 2 - bond_length) * std::sin(current_rand0 * 2 * M_PI));
        //疎水基2_write_vel
        fprintf(fpo1, "%d   %lf   %lf   %lf \n", particle_id + 1, 0.0, 0.0, 0.0);
        //疎水基3＋疎水基2_write_bond
        fprintf(fpo2, "%d   %d   %d \n", particle_id, particle_id + 1, type_bond_a);
        ...
        //疎水基1_write_pos
        fprintf(fpo0, "%d %d   %lf   %lf   %lf \n", particle_id + 2, type_lipid_b,
                (vesicle_core_pos_x + vesicle_radius - lipid_to_lipid_length / 2 - bond_length * 2) * std::cos(current_rand0 * 2 * M_PI) * std::cos(current_rand2 * M_PI),
                (vesicle_core_pos_y + vesicle_radius - lipid_to_lipid_length / 2 - bond_length * 2) * std::sin(current_rand2 * M_PI) * std::cos(current_rand0 * 2 * M_PI),
                (vesicle_core_pos_z + vesicle_radius - lipid_to_lipid_length / 2 - bond_length * 2) * std::sin(current_rand0 * 2 * M_PI));
        //疎水基1_write_vel
        fprintf(fpo1, "%d   %lf   %lf   %lf \n", particle_id + 2, 0.0, 0.0, 0.0);
        //疎水基2＋疎水基1_write_bond
        fprintf(fpo2, "%d   %d   %d \n", particle_id + 1, particle_id + 2, type_bond_a);
        ...
        //親水基_write_pos
        fprintf(fpo0, "%d %d   %lf   %lf   %lf \n", particle_id + 3, type_lipid_a,
                (vesicle_core_pos_x + vesicle_radius - lipid_to_lipid_length / 2 - bond_length * 3) * std::cos(current_rand0 * 2 * M_PI) * std::cos(current_rand2 * M_PI),
                (vesicle_core_pos_y + vesicle_radius - lipid_to_lipid_length / 2 - bond_length * 3) * std::sin(current_rand2 * M_PI) * std::cos(current_rand0 * 2 * M_PI),
                (vesicle_core_pos_z + vesicle_radius - lipid_to_lipid_length / 2 - bond_length * 3) * std::sin(current_rand0 * 2 * M_PI));
        //親水基_write_vel
        fprintf(fpo1, "%d   %lf   %lf   %lf \n", particle_id + 3, 0.0, 0.0, 0.0);
        //疎水基1＋親水基_write_bond
        fprintf(fpo2, "%d   %d   %d \n", particle_id + 2, particle_id + 3, type_bond_a);
...
//    std::cout << particle_id << std::endl;
  ...
  for (count_num_outer_lipid_xy = 0; count_num_outer_lipid_xy < num_outer_lipid_xy; count_num_outer_lipid_xy++)
  {
    temp_angle_xy += angle_to_place_outer_lipid;
...
//    std::cout << particle_id << std::endl;
  ...
  while (temp_num_outer_water < num_outer_water)
  {
    ...
    //ベシクルの外側に存在する水粒子
    if (powf(random_x - vesicle_core_pos_x, 2.0) + powf(random_y - vesicle_core_pos_y, 2.0) + powf(random_z - vesicle_core_pos_z, 2.0) > vesicle_outer_radius2 && temp_num_outer_water < num_outer_water)
    {
      //_write_pos
